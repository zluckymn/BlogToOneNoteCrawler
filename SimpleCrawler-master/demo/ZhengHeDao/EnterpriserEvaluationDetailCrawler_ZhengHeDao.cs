...
namespace SimpleCrawler.Demo
{
    ...
    /// <summary>
    /// 口碑1
    /// http://www.zhisland.com/bms-api-app/comment/6074390067441303556?count=20&nextId=1
    ///
    /// </summary>
    public class EnterpriserEvaluationDetailCrawler_ZhengHeDao : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.isUpdate”已被赋值，但从未使用过它的值
        ...
        public EnterpriserEvaluationDetailCrawler_ZhengHeDao(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "User_Evaluation_ZhengHeDao";//动态
            DataTableCategoryName = "User_ZhengHeDao";
            updatedValue = "1";//是否更新字段
            uniqueKeyField = "guid";
            updatedField = "isEvaluationDeail";
        ...
        public void initialUrl()
        {
            ...
            var userlist=FindDataForUpdate(dataTableName: DataTableCategoryName, query:Query.Exists("isEvaluationDeail", false));
            foreach (var user in userlist)
            {
                var guid = user.Text("guid");
                var curUrl = $"http://www.zhisland.com/bms-api-app/comment/{guid}?count=20";
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.countPerFolder”已被赋值，但从未使用过它的值
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            try
            {
                ...
                var dataList = jobject["data"];
                if (dataList != null)
                {
                    foreach (var data in dataList)
                    {
                        var bsonDoc = data.ToString().GetBsonDocFromJson();
                        bsonDoc.Set("guid", bsonDoc.Text("feedId"));
                        bsonDoc.Set("userId", guid);
                        //AddData(bsonDoc);
                        PushData(bsonDoc);
                        ShowStatus();
                        var publisher = bsonDoc.GetBsonDocument("publisher");
                        if (publisher != null && publisher.ElementCount > 0)
                        {
                            publisher.Set("guid", publisher.Text("userId"));
                            publisher.Set("isEvaluationAdd", 1);
                            PushData(publisher, dataTable: DataTableCategoryName);
                        }
                    }
                    if (dataList.Count() >= 20 && jobject["nextId"] != null)
                    {
                        //下一页
                        var nextId = jobject["nextId"].ToString();
                        if (!string.IsNullOrEmpty(nextId))
                        {
                            var url = ReplaceUrlParam(args.Url, "nextId", nextId);
                            AddUrl(url, guid);
                        }
                    }
                }
                UpdateDataStatusOnly(dataTable: DataTableCategoryName, Query.EQ("guid", guid));
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            try
            {
                ...
                if (args.Html.Contains("nextId"))//需要编写被限定IP的处理
                {
                    return false;
                }
                else
