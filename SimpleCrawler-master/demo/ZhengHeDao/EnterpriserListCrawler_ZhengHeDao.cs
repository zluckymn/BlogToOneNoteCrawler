...
namespace SimpleCrawler.Demo
{
    ...
    ///
    /// </summary>
    public class EnterpriserListCrawler_ZhengHeDao : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.isUpdate”已被赋值，但从未使用过它的值
        ...
        public EnterpriserListCrawler_ZhengHeDao(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "User_ZhengHeDao";//房间
            DataTableCategoryName = "Industry_ZhengHeDao";
        ...
        public void initialUrl()
        {
            ...
            var url = $"http://www.zhisland.com/bms-api-app/search/company?count=20";
            //var curUrl = $"https://www.baidu.com";
          
            var parentIndustryList = industryList.Where(c => c.Text("parentCode")== "").OrderBy(c=>c.Text("name")).ToList();
             foreach (var industry in parentIndustryList) {
                var code = industry.Text("code");
                var subIndustryList = industryList.Where(c => c.Text("parentCode") == code).ToList();
                foreach (var subIndustry in subIndustryList)
                {
                    var subCode = subIndustry.Text("code");
                    var curUrl = ReplaceUrlParam(url, "industry", subCode);
                    UrlQueue.Instance.EnQueue(new UrlInfo(curUrl) {  UniqueKey= subCode });
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.countPerFolder”已被赋值，但从未使用过它的值
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            try
            {
                ...
                {
                    var bsonDoc = dObj.ToString().GetBsonDocFromJson();
                    if (!string.IsNullOrEmpty(industryCode))
                    {
                        bsonDoc.Set("industryCode", industryCode);
                    }
                    bsonDoc.Set("userId", bsonDoc.Text("userId"));
                    bsonDoc.Set("guid", bsonDoc.Text("userId"));
                    //AddData(bsonDoc);
                    PushData(bsonDoc);
                }
                ShowStatus();
                //新增分页?area=110000&industry=ind_01&financing=financing_stage_01&scale=enterprise_employee_num_5&count=20
                var filter = jObject["filter"];

                GetMoreUrl(filter, "area", args);
                GetMoreUrl(filter, "financing", args);
                GetMoreUrl(filter, "scale", args);
                GetMoreUrl(filter, "uType", args);
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            try
            {
                ...
                if (args.Html.Contains("code"))//需要编写被限定IP的处理
                {
                    return false;
                }
                else
