...
namespace SimpleCrawler.Demo
{
    ...
    ///
    /// </summary>
    public class EnterpriserFeedDetailCrawler_ZhengHeDao : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.isUpdate”已被赋值，但从未使用过它的值
        ...
        public EnterpriserFeedDetailCrawler_ZhengHeDao(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "User_Feed_ZhengHeDao";//动态
            DataTableCategoryName = "User_ZhengHeDao";
            updatedValue = "1";//是否更新字段
            uniqueKeyField = "guid";
            updatedField = "isFeedDeail";
        ...
        public void initialUrl()
        {
            ...
            var userlist=FindDataForUpdate(dataTableName: DataTableCategoryName, query:Query.Exists("isFeedDeail", false));
            foreach (var user in userlist)
            {
                var guid = user.Text("guid");
                var curUrl = $"https://www.zhisland.com/bms-api-app/feed/user/{guid}?count=20&uid={guid}";
