...
{
    /// <summary>
    /// 获取美团医疗美术数据,目前仅获取北京数据
    ///https://i.meituan.com/beauty/medical/channel/shop/ajax/getshops?source=mt&cityid=1&lng=118.046779&lat=24.610114&dpid=&uuid=&utmmedium=&pageno=3&pagesize=10&roottagid=86&tagid=79&around=0&adnum=0&qsnum=0&originas=&originqs=
    ///  </summary>
    public class MeiTuCityBusinessEnterpriseAPPCrawler_MedicBeau : ISimpleCrawler
    {
        ...
        private static string DateStr = DateTime.Now.ToString("yyyy_MM_dd");
        private static string _DataTableName = "Enterprise_MT_MedicalBeautify_BJ";//存储的数据库表明
        ...
        {
            get { return "CityCategoryName_MT_MedicalBeautify" ; }
        ...
        public MeiTuCityBusinessEnterpriseAPPCrawler_MedicBeau(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop)
        {
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
           ...
           // Settings.hi.CurlObject.SetOpt(LibCurlNet.CURLoption.CURLOPT_PROXY, Settings.CurWebProxyString);
            ...
            var allCategory = dataop.FindAll(DataTableNameCityCategory).Where(c=>c.Text("level")=="3"&&c.Text("tagName")!="推荐").Select(c=>c.Text("tagId")).Distinct().ToList();
            ...
            foreach (var tagId in allCategory)
            {
                var cityId = 1;
                var pageno =1;
                var pagesize = 10;
                var roottagid = 1;
                var hitEnterpriseUrl = $"https://i.meituan.com/beauty/medical/channel/shop/ajax/getshops?source=mt&cityid={cityId}&lng=&lat=&dpid=&uuid=&utmmedium=&pageno={pageno}&pagesize={pagesize}&roottagid={roottagid}&tagid={tagId}&around=0&adnum=0&qsnum=0&originas=&originqs= ";


                if (!filter.Contains(hitEnterpriseUrl)) { 
                 //生成爬取详细信息链接
