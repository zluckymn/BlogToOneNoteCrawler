...
namespace SimpleCrawler.Demo
{
    ...
    /// 用于城市与区域代码初始化
    /// </summary>
    public class ProvincePlantListCrawler : ISimpleCrawler
    {
        ...
        public ProvincePlantListCrawler(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop)
        {
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            foreach (var province in provinceList)
            {
                ...
                buildingList = dataop.FindAllByQuery(fixDataTable, Query.NE("isDetailUpdate", 1)).Where(c => c.Int("type") == 0).ToList();
                //iCPH
                foreach (var building in buildingList)
                {
                    var mhUrl = string.Format("http://frps.eflora.cn/protreeajax.aspx?ID={0}&p={1}", building.Text("id"), provinceId);
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            if (aNodes != null)
            {
                ...
                foreach (var aNode in aNodes)
                {
                    ...
                    DBChangeQueue.Instance.EnQueue(new StorageData() { Document = new BsonDocument("isDetailUpdate", 1), Name = fixDataTableName, Query = Query.EQ("id", nodePid), Type = StorageType.Update });
                }
                Console.WriteLine("新增{0}更新:{1}{2}", add, update,provinceName);
