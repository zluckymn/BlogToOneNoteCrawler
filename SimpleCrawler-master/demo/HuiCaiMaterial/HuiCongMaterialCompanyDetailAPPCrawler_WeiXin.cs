...
namespace SimpleCrawler.Demo
{
    ...
    /// 
    ///  </summary>
    public class HuiCongMaterialCompanyDetailAPPCrawler_WeiXin : SimpleCrawlerBase
    {
        ...
        public HuiCongMaterialCompanyDetailAPPCrawler_WeiXin(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop):base(_Settings, _filter, _dataop)
        {
            DataTableName = "Material_Supplier_HuiCong_WLM";//房间
        ...
        string materialUrl = "https://wsmobile.hc360.com/companyinfo?pid={0}";
        ...
        public void initialUrl()
        {
            ...
            foreach (var matObj in hitMatList)
            {
                ...
                var curUrl = string.Format(materialUrl, matObj.Text("guid"));

                if (!filter.ContainsAdd(curUrl))
                {
                    UrlQueue.Instance.EnQueue(new UrlInfo(curUrl) { Depth = 1, UniqueKey = matObj.Text("guid") });
                }
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            try
            {
                ...
                if (args.Html.Contains("securityBond"))//需要编写被限定IP的处理
                {
                    return false;
                }
                else
