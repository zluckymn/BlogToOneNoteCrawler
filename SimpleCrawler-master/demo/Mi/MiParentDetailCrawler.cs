...
namespace SimpleCrawler.Demo
{
    ...
    /// 迷途
    /// </summary>
    public class MiParentDetailCrawler : ISimpleCrawler
    {
        ...
        public MiParentDetailCrawler(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop)
        {
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            var allObjList = dataop.FindAllByQuery(DataTableName,Query.NE("isUpdate",2)).SetFields("pIds","id").ToList();
...
#pragma warning restore CS0219 // 变量“allPage”已被赋值，但从未使用过它的值
            //foreach (var region in regionList)
            {
                ...
                {
                    if (!obj.ContainsColumn("pIds")) continue;
                    var pids= obj["pIds"]as BsonArray;
                    foreach (var pid in pids) {
                        var url = string.Format("https://jiamitu.mi.com/pet/detail?petId={0}&followUp=https:%2F%2Fjiamitu.mi.com%2Fmitudetail%3FpetId%3D{0}", pid.ToString());
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            var childId = args.urlInfo.UniqueKey;
            if (result == null) return;
                ...
                if (!idFilter.Contains(guid) && !hasExistObj(guid))
                {
                    insert++;
                    idFilter.Add(guid);
                    document.Set("childId", childId);
                    document.Set("isUpdate", 1);
                    DBChangeQueue.Instance.EnQueue(new StorageData() { Document = document, Name = DataTableNameDetail, Type = StorageType.Insert });
                ...
                DBChangeQueue.Instance.EnQueue(new StorageData() { Document = new BsonDocument().Add("isUpdate",2), Name = DataTableName, Query=Query.EQ("id",childId), Type = StorageType.Update });
