...
namespace SimpleCrawler.Demo
{
    ...
    ///
    /// </summary>
    public class KuaiKan_ListCrawler : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“PositionListCrawler_LiePin.isUpdate”已被赋值，但从未使用过它的值
        ...
        public KuaiKan_ListCrawler(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "ManHua_KuaiKan";//房间
            DataTableCategoryName = "ManHua_Category_KuaiKan";//房间
            updatedValue = "1";//是否更新字段
            uniqueKeyField = "guid";
            allmanHuaList = dataop.FindAll(DataTableName).SetFields("guid", "tagIds").ToList();
        }
        ...
        public void initialUrl()
        {
            ...
            var tags = dataop.FindAll(DataTableCategoryName).SetFields("name", "guid", "id").ToList();
            foreach (var hitTag in tags)
            {
                var id = hitTag.Text("id");
                var key = id;
                var type = hitTag.Text("name");
                var url = $"https://www.kuaikanmanhua.com/v1/search/by_tag?since=0&count={takeCount}&f=3&tag={id}&sort=1&query_category=%7B%22update_status%22:1%7D";
...
#pragma warning restore CS0414 // 字段“PositionListCrawler_LiePin.noCountTimes”已被赋值，但从未使用过它的值
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            if (root == null) return;
            var total = root.Int("total");//总页数
            var data = root.GetBsonDocument("data");
            if (data != null)
            {
                var topics= data.GetBsonDocumentList("topics");

                foreach (var topic in topics)
                {
                    var hitObj = allmanHuaList.Where(c => c.Text("guid") == topic.Text("id")).FirstOrDefault();
                    if (hitObj != null)
                    {
                        var tagIds = hitObj.ContainsColumn("tagIds") ? hitObj["tagIds"] as BsonArray : new BsonArray();
                        if (!tagIds.Contains(tagValue))
                        {
                            tagIds.Add(tagValue);
                            topic.Set("tagIds", tagIds);
                            hitObj.Set("tagIds", tagIds);
                        }
                    ...
                    else
                    {
                        ...
                        topic.Set("tagIds", BsonArray.Create(new List<string> { tagValue }));
                        allmanHuaList.Add(topic);
                    }
            ...
            if (total >takeCount)
            {
                ...
                for (var count = takeCount; count <= total; count += takeCount)
                {
                   var  hitUrl = ReplaceUrlParam(url, "since", count.ToString(),"?");
                    if (!filter.Contains(hitUrl))
                    {
                        UrlQueue.Instance.EnQueue(new UrlInfo(hitUrl) { UniqueKey = args.urlInfo.UniqueKey, extraData = args.urlInfo.extraData });
                        filter.Add(url);// 防止执行2次
                    }
