...
namespace SimpleCrawler.Demo
{
    ...
    /// <summary>
    ///  
    ///https://social.kkmh.com/v1/graph/users/110255565
    ///
    /// </summary>
    public class KuaiKan_UserSimulateDetailCrawler : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“PositionDetailCrawler_LiePin.isUpdate”已被赋值，但从未使用过它的值
        ...
        public KuaiKan_UserSimulateDetailCrawler(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableCategoryName = "ManHua_User";//房间
            DataTableName = "ManHua_User_Simulate";//房间
            updatedValue = "1";//是否更新字段
            uniqueKeyField = "guid";
        }
        List<BsonDocument> allHitObjList;
        BloomFilter<string> _idFilter = new BloomFilter<string>(2000000);
        public void initialUrl()
        {
            var mongoDb= MongoOpCollection.GetNew121MongoOp_MT("AppActivity");
            var allHitObjList_temp = mongoDb.FindAll(DataTableCategoryName).SetFields("id").Select(c=>c.Int("id")).ToList();
            var maxCount = allHitObjList_temp.Max(c => c);
            //(dataTableName: DataTableName, fields:new string[] { "guid" },limit:10000);
            //初始化布隆过滤器
            /// 在每个数值前后加100 -100
            foreach (var guid in allHitObjList_temp)
            {

                _idFilter.Add(guid.ToString());
            }
            var resultIds = new List<int>();
            foreach (var hitId in allHitObjList_temp)
            {
                resultIds.AddRange(SimulateIds(hitId));
            }
            foreach(var guid in resultIds.Where(c=>c>0))
            {
                
                var url = $"https://social.kkmh.com/v1/graph/users/{guid}";
                if (!filter.Contains(url))
                {
                    UrlQueue.Instance.EnQueue(new UrlInfo(url) { UniqueKey = guid.ToString()});
                    filter.Add(url);// 防止执行2次
                }
            }
        }
        private List<int> SimulateIds(int id)
        {
            var preId = id;
            var nextId = id;
            ...
            for (var i = 0; i <= 2; i++)
            {
                --preId;
                ++nextId;
                if (!_idFilter.Contains(preId.ToString()))
                {
                    result.Add(preId);
                    _idFilter.Add(preId.ToString());
                }
                if (!_idFilter.Contains(nextId.ToString()))
                {
                    result.Add(nextId) ;
                    _idFilter.Add(nextId.ToString());
                }
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            Settings.Referer = "Expires=Fri, 24-Jul-2020 12:10:04 GMT;Max-Age=86400;kk_s_t=1595821284426;Domain=.kkmh.com;Path=/";
...
#pragma warning restore CS0414 // 字段“PositionDetailCrawler_LiePin.noCountTimes”已被赋值，但从未使用过它的值
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            //    initialUrl();
            //}
            var guid = args.urlInfo.UniqueKey;//用户主键
            ...
            if (data != null)
            {
                data.Set("isSimulate", 1);
                data.Set("guid", guid);
               data.Set(updatedField, updatedValue);
               PushData(data);
            }
