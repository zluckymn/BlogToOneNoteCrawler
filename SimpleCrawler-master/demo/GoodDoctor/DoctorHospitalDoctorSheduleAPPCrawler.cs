...
namespace SimpleCrawler.Demo
{
    ...
    /// 
    ///  </summary>
    public class DoctorHospitalDoctorSheduleAPPCrawler : ISimpleCrawler
    {
...
#pragma warning restore CS0414 // 字段“DoctorHospitalDoctorSheduleAPPCrawler.pageIndex”已被赋值，但从未使用过它的值
        ...
        string reqtoken = "863324321401123";
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            var allDoctorList = dataop.FindAllByQuery(DataTableNameDoctor,Query.NE("isScheduleUpdate", "1")).SetFields("guid").ToList();
            foreach (var doctor in allDoctorList)//法庭
            {
                 var postData = string.Format("app=p&os=android&n=2&m=GT-I9300&v=5.2.5&di={0}&s=hd&doctorId={1}&deviceToken={0}&p=1&userId=0&currentUserId=0&sv=4.4.2&api=1.2",reqtoken, doctor.Text("guid"));
                 UrlQueue.Instance.EnQueue(new UrlInfo(materialUrl) { Depth = 1, PostData = postData });
                if(!guidFilter.Contains(doctor.Text("guid")))
                guidFilter.Add(doctor.Text("guid"));
            }
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            var doctorId = GetUrlParam(args.urlInfo.PostData, "doctorId") ;
            if (docList != null)
            {
                var insert = 0;
                var update = 0;
                
                    BsonDocument doctorDoc = MongoDB.Bson.Serialization.BsonSerializer.Deserialize<BsonDocument>(docList.ToString());
                   
                    var guid = doctorId;
                    doctorDoc.Set("isScheduleUpdate","1");
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            ...
            if (html.Contains("页面没有找到")||html.Contains("Object moved")|| html.Contains("Service Unavailable") )//需要编写被限定IP的处理
            {
                return true;
            }
