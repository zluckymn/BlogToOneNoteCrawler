...
namespace SimpleCrawler.Demo
{
    ...
    /// 
    ///  </summary>
    public class DoctorHospitalDoctorListAPP : ISimpleCrawler
    {
       ...
       // private   string connStr = "mongodb://MZsa:MZdba@59.61.72.34:37088/SimpleCrawler";
        ...
        public DoctorHospitalDoctorListAPP(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop)
        {
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            //Settings.Depth = 4;
            //代理ip模式
            //Settings.IPProxyList =IPProxyHelper.GetIpProxyList("2");
            ...
            var allHospitalList = dataop.FindAll(DataTableName).SetFields("facultList","guid","name").ToList();
            var allDoctorList= dataop.FindAll(DataTableNameDoctor).SetFields("hospitalFacultyId", "guid").ToList();
            foreach (var facltsDoc in allHospitalList)//法庭
            {
                var facultList = facltsDoc.Text("facultList").ToString();
                facultList =  "{\"data\":" + facultList + "}";
                JObject facult = JObject.Parse(facultList);


                var typeList = facult["data"];
                foreach (var type in typeList.ToList())
                {
                    var subFacultyList = type["subFacultyList"];

                    foreach (var subFaculty in subFacultyList.ToList())
                    {
                        var doctorCount = int.Parse(subFaculty["doctorCount"].ToString());
                        //此处需要判断医生个数是否与现有的匹配，小与等于后继续进行爬取
                        var existDoctorCount = allDoctorList.Where(c => c.Text("hospitalFacultyId") == subFaculty["hospitalFactoryId"].ToString()).Count();
                        if (existDoctorCount >= doctorCount)
                        {
                            Console.WriteLine("{0}该科室{1}已爬取完", facltsDoc.Text("name"), subFaculty["name"]);
                            continue;
                        }
                        var pageCount = (doctorCount-1) / pageSize+1;
                        if (pageCount <= 0)
                            pageCount = 1;

                        for (var index = 1; index <= pageCount; index++)
                        {
                            var postData = string.Format("app=p&os=android&hospitalFacultyId={0}&pageId={1}&pageSize={2}&n=2&m=GT-I9300&serviceArr=&v=5.2.5&di={3}&s=hd&doctorGrade=&deviceToken={3}&p=1&userId=&currentUserId=0&sv=4.4.2&api=1.2&doctorScheduleArr=&hospitalId={4}&facultyId={5}", subFaculty["hospitalFactoryId"], index, pageSize, reqtoken, facltsDoc.Text("guid"), subFaculty["facultyId"]);
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            //获取查询条件
            var docList = jsonObj["data"]["content"];
            var hospitalFacultyId = GetUrlParam(args.urlInfo.PostData, "hospitalFacultyId");
            var facultyId = GetUrlParam(args.urlInfo.PostData, "facultyId");
            var hospitalId  = GetUrlParam(args.urlInfo.PostData, "hospitalId");
            if (docList != null)
            {
                var insert = 0;
                var update = 0;
                Console.WriteLine("获得数据:{0}", docList.ToList().Count);
                ...
                foreach (var doctor in docList)
                {
                    ...
                    {
                        doctorDoc.Add("hospitalId", hospitalId);//医院
                        doctorDoc.Add("hospitalFacultyId", hospitalFacultyId);//科室
                        doctorDoc.Add("facultyId", facultyId);//医院
                    ...
                    {
                        DBChangeQueue.Instance.EnQueue(new StorageData() { Document = doctorDoc, Name = DataTableNameDoctor,Query=Query.EQ("guid", guid), Type = StorageType.Update });
                        update++;
                    }
                }
