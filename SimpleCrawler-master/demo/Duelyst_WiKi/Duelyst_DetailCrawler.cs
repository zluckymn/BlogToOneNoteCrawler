...
namespace SimpleCrawler.Demo
{
    ...
    ///
    /// </summary>
    public class Duelyst_DetailCrawler : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“PositionDetailCrawler_LiePin.isUpdate”已被赋值，但从未使用过它的值
        ...
        public Duelyst_DetailCrawler(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "DuelystCardDetail";//房间
            DataTableCategoryName = "DuelystCards";//房间
        ...
        public void initialUrl()
        {
            ...
            foreach (var hitObj in allHitObjList)
            {
                ...
                var url = $"https://duelyst.gamepedia.com{hitObj.Text("href")}";
                if (!filter.Contains(url))
                {
                    UrlQueue.Instance.EnQueue(new UrlInfo(url) { UniqueKey = guid, extraData= hitObj.Text("name")});
...
#pragma warning restore CS0414 // 字段“PositionDetailCrawler_LiePin.noCountTimes”已被赋值，但从未使用过它的值
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            ...
            var cardName = args.urlInfo.extraData;
            if (root == null) return;
            var hitANodes = root.DocumentNode.SelectNodes("//a").Where(c =>  c.InnerHtml.Contains("gif")).ToList();
           
            if (hitANodes != null)
            {
                var imgFolderName = $"Duelyst/{cardName}";
                foreach (var aNode in hitANodes)
                {
                    var imgNode = aNode.ChildNodes.Where(c => c.Name == "img").FirstOrDefault();
                    if (imgNode == null)
                    {
                        continue;
                    }
                    var imgSrc = GetNodeAttribute(imgNode, "src");
                    var alt= GetNodeAttribute(imgNode, "alt");
                    var href = GetNodeAttribute(aNode, "href");
                    var parentDiv = aNode.ParentNode;
                    var action = parentDiv.InnerText.Trim();
                    
                    var bsonDoc = new BsonDocument();
                    bsonDoc.Set("action", action);
                    bsonDoc.Set("alt", alt);
                    bsonDoc.Set("src", imgSrc);
                    bsonDoc.Set("fileName", alt.Replace(" ", "_"));
                    bsonDoc.Set("src_local", $"{imgFolderName}/{bsonDoc.Text("fileName") }");
                    bsonDoc.Set("href", href);
                    bsonDoc.Set("cardGuid", guid);
                    bsonDoc.Set("guid", alt.EncodeMD5().ToLower());
                    ...
                    PushData(bsonDoc, addAction: (doc) => {
                        ...
                        {
                            DownLoadFile(src, newFileName, imgFolderName);
                        }
                    });
                }
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            try
            {
                if (args.Html.Contains("File:"))//需要编写被限定IP的处理
                {
                    return false;
                }
                else
