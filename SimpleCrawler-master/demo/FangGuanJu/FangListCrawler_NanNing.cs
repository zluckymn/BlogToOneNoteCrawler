...
namespace SimpleCrawler.Demo
{
    ...
    /// 江阴房管局列表爬取
    /// </summary>
    public class FangListCrawler_NanNing : ISimpleCrawler
    {
        ...
        public FangListCrawler_NanNing(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop)
        {
        ...
        public void SettingInit()//进行Settings.SeedsAddress Settings.HrefKeywords urlFilterKeyWord 基础设定
        {
            ...
            for(var pageIndex=1; pageIndex<=527; pageIndex++)
                {
                    var url = string.Format("http://www.nnfcxx.com/vipdata/fcj.php?typename=xmmc&list=xjspf&xm_value=&page={0}", pageIndex);
            ...
            if (trNodes != null)
            {
                ...
                foreach (HtmlNode aNode in trNodes)
                {
                    ...
                    if (aNode != null && aNode.Attributes["onclick"] != null)
                    {
                        ...
                        var projId = Toolslib.Str.Sub(url, "&bh=", "");
                        var curDoc = new BsonDocument();
                        curDoc.Add("projId", projId);
                        curDoc.Add("companyName", tdNodes[0].InnerText.Trim());
                        curDoc.Add("name", tdNodes[1].InnerText.Trim());
                        curDoc.Add("address", tdNodes[2].InnerText.Trim());
                        curDoc.Add("saleNo", tdNodes[3].InnerText.Trim());
                        curDoc.Add("saleArea", tdNodes[4].InnerText.Trim());
                        curDoc.Add("saleDate", tdNodes[5].InnerText.Trim());
                        curDoc.Add("url", url);
