...
namespace SimpleCrawler.Demo
{
    ...
    /// </summary>
    public class EnterpriserSearchCrawler_ShunQi : SimpleCrawlerBase
    {
...
#pragma warning restore CS0414 // 字段“EnterpriserListCrawler_ZhengHeDao.isUpdate”已被赋值，但从未使用过它的值
        ...
        public EnterpriserSearchCrawler_ShunQi(CrawlSettings _Settings, BloomFilter<string> _filter, DataOperation _dataop) : base(_Settings, _filter, _dataop)
        {
            DataTableName = "Enterprise_QiShun";//房间
        ...
        public void DataReceive(DataReceivedEventArgs args)
        {
            try
            {
                ...
                var companylist = htmlDoc.SelectNodes("//div[@class='result f s0']");
                foreach (var company in companylist)
                {
                    var bsonDoc = new BsonDocument();

                   
                    var urlNode = company.SelectSingleNode("./h3/a");
                    if (urlNode != null)
                    {
                        var url = GetNodeAttribute(urlNode, "href");
                        var ent_name = urlNode.InnerText.Replace("<em>","").Replace("</em>","");
                        if (name != ent_name)
                        {
                            continue;
                        }
        ...
        public bool IPLimitProcess(DataReceivedEventArgs args)
        {
            try
            {
                ...
                if (args.Html.Contains("为您找到相关结果"))//需要编写被限定IP的处理
                {
                    return false;
                }
                else
