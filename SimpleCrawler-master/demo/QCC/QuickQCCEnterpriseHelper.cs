...
namespace MZ.QuickHelper
{
    ...
    /// 通过模拟账号App远程获取数据
    /// </summary>
    public class QuickQCCEnterpriseHelper
    {
        /// <summary>
        /// 企查查app账号表
        /// </summary>
        ...
        public QuickQCCEnterpriseHelper()
        {
        ...
        public string GetAccessToken()
        {
           ...
           // var postData = string.Format("appId={0}&deviceId={1}&version=9.2.0&deviceType=android&os=&timestamp={2}&sign={3}", curDeviceInfo.appId, curDeviceInfo.deviceId, curDeviceInfo.timestamp, curDeviceInfo.sign);
            var postData = string.Format("appId={0}&deviceId={1}&version=10.0.4&deviceType=android&os=&timestamp={2}&sign={3}", curDeviceInfo.appId, curDeviceInfo.deviceId, curDeviceInfo.timestamp, curDeviceInfo.sign);
...
#pragma warning restore CS0168 // 声明了变量“ex”，但从未使用过
        ...
        List<BsonDocument> allDeviceAccountList = new List<BsonDocument>();
        public List<BsonDocument> ReloadDeviceAccount()
        {
            var dataop = MongoOpCollection.Get121MongoOp("SimpleCrawler");
            var query = Query.And(Query.NE("isInvalid", "1"));
              allDeviceAccountList = dataop.FindAll(QCCDeviceAccount, query).SetSortOrder(new SortByDocument() { { "updateDate", 1 } }).SetLimit(1000).ToList();
            return allDeviceAccountList;
        }

        public DeviceInfo AutoChangeDeviceAccount()
        {
            ReloadDeviceAccount();
            if (allDeviceAccountList.Count() > 0)
            {
                var oldDeviceId = curDeviceInfo.deviceId;
                var hitAccountList = allDeviceAccountList.Where(c => c.Text("deviceId") != curDeviceInfo.deviceId).ToList();
                #region 随机获取数据
                var rnd = new Random();
        ...
        /// <returns></returns>
        public DeviceInfo DocToDeviceInfo(BsonDocument doc)
        {
            curDeviceInfo = new DeviceInfo()
            {
                accessToken = doc.Text("accessToken"),
                appId = doc.Text("appId"),
                deviceId = doc.Text("deviceId"),
                deviceToken = doc.Text("deviceToken"),
                refreshToken = doc.Text("refreshToken"),
                timestamp = doc.Text("timestamp"),
                sign = doc.Text("sign")
            };
            return curDeviceInfo;
        }
        public bool SetSetting(BsonDocument doc)
        {
            string deviceId = doc.Text("deviceId");
            string timestamp = doc.Text("timestamp");
            string sign = doc.Text("sign");
            string refleshToken = doc.Text("refleshToken");
            string accessToken = doc.Text("accessToken");
        ...
        public bool SetSetting(string deviceId, string timestamp, string sign, string refleshToken, string accessToken)
        {
            ...
            {
                curDeviceInfo.deviceId = deviceId.Trim();
            }
            if (!string.IsNullOrEmpty(timestamp))
            {
                curDeviceInfo.timestamp = timestamp.Trim();
            }
            if (!string.IsNullOrEmpty(sign))
            {
                curDeviceInfo.sign = sign.Trim();
            }

            curDeviceInfo.refreshToken = refleshToken.Trim();
            curDeviceInfo.accessToken = accessToken.Trim();

            if (true || string.IsNullOrEmpty(curDeviceInfo.accessToken))
            {
                var result = RefreshToken(true);//强制刷新
                if (result.Contains("成功"))
                {
                    Console.WriteLine(string.Format("DeviceId:{0},timestamp:{1},sign:{2},RefleshToken:{3},AccessToken:{4}", curDeviceInfo.deviceId, curDeviceInfo.timestamp, curDeviceInfo.sign, curDeviceInfo.refreshToken, curDeviceInfo.accessToken), false);
                    return true;
                }
            }
            return false;
